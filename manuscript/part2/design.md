...
![Equivalent tests using a ports and adapters approach](missing.png)
...
When a user requests the current price in the UI, the domain model is asked for the current price and responds accordingly, so that the user can see the latest price (Abstract tag line, what's happening)
